...
def ids_solve(start: str, goal: str, words: List[str], graph: Dict[str, List[str]], stop_flag=lambda: False, max_depth: int = 50):
    ...
    for depth_limit in range(1, max_depth + 1):
        ...
        if dfs_limited(start, goal, graph, depth_limit, visited, path):
            return path

    return None


def dfs_limited(u, goal, graph, depth_left, visited, path):
    """
    ...
    for v in graph[u]:
        if v not in visited:
            if dfs_limited(v, goal, graph, depth_left - 1, visited, path):
                return True
